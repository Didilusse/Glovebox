...
describe('auth flows', () => {
  ...
  it('keeps the account signed in after changing its password', async () => {
    ...
    await wrapper.find('form').trigger('submit')
    await flushPromises()
    expect(JSON.parse(fetch.mock.calls[0][1].body)).toEqual({ current_password: ' old pass ', new_password: ' new password ' })
  ...
  it('creates users, confirms password reset, and requires confirmation before deletion', async () => {
    ...
    await wrapper.findAll('button').find(button => button.text() === 'Add new user').trigger('click')
    const inputs = wrapper.find('[role="dialog"]').findAll('input')
  ...
  it('can cancel deletion and reports admin API errors without removing users', async () => {
    ...
    expect(wrapper.findAll('.users-table tbody tr')).toHaveLength(2)
  })
})
