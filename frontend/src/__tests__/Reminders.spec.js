...
describe('reminder cards and dashboard', () => {
  ...
  it('shows only configured reminders, a due badge and keyboard-operable tabs', async () => {
    ...
    const router = await render(CarDetailView, {}, '/car/car-1')
    expect(wrapper.find('#reminders-tab').text()).toContain('1 due')
    expect(wrapper.find('.status-card').text()).toContain('1 service needs attention')
    ...
    expect(router.currentRoute.value.query.tab).toBe('reminders')
