...
async def create_maintenance_log(car_id: str, log_data: MaintenanceLogCreate):
    ...
    if log_dict["parts"]:
        log_dict["cost"] = sum(part.get("cost") or 0 for part in log_dict["parts"])
...
async def add_part_to_log(car_id: str, log_id: str, part_data: Parts):
    car = await CarModel.get(PydanticObjectId(car_id))
    if not car:
        raise HTTPException(status_code=404, detail="Vehicle not found")
...
async def delete_part_from_log(car_id: str, log_id: str, part_index: int):
    car = await CarModel.get(PydanticObjectId(car_id))
    if not car:
        raise HTTPException(status_code=404, detail="Vehicle not found")
