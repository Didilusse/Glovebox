...
def test_admin_lists_and_creates_users(api_client):
    ...
    weak = api_client.post("/users/", json={"username": "helper", "password": "12345678901"})
    assert weak.status_code == 422


def test_non_admin_cannot_manage_users(api_client):
    create_user(api_client, "mechanic")
    mechanic = auth_headers_for(api_client, "mechanic", "password1234")

    assert api_client.get("/users/", headers=mechanic).status_code == 403
    assert api_client.post(
        "/users/", json={"username": "helper", "password": "password1234"}, headers=mechanic
    ).status_code == 403
...
def test_admin_resets_password_and_revokes_sessions(api_client):
    ...
    assert api_client.post(
        "/auth/login", json={"username": "mechanic", "password": "password1234"}, headers={}
    ).status_code == 401
    assert api_client.post(
    ...
    assert api_client.patch(
        f"/users/{created['_id']}/password", json={"new_password": "12345678901"}
    ).status_code == 422
    assert api_client.patch(
